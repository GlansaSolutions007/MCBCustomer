...
const ConfirmAddressPage = ({ navigation }) => {
    ...
    useEffect(() => {
        const fetchStatesAndCities = async () => {
            try {
                ...
                const cityRes = await fetch(`${API_BASE_URL}City`);
    ...
    const reverseGeocode = async () => {
        ...
        if (data.results?.length > 0) {
            ...
            const matchedState = statesList.find(s => s.StateName.toLowerCase() === state.toLowerCase());
    ...
    const submitAddress = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${API_BASE_URL}CustomerAddresses`, payload, {
                headers: {
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }} edges={['bottom']}>
            <View style={{ flex: 1, backgroundColor: "#fff" }}>
                    ...
                    onPress={async () => {
                        try {
                            ...
                            if (status !== 'granted') {
                                ...
                                setAlertMessage("Location permission is required to use this feature.");
                                setAlertVisible(true);
                                return;
                            }
                            let location = await Location.getCurrentPositionAsync({});
                ...
                >
                    <MaterialIcons name="my-location" size={20} color={color.secondary} style={{ marginRight: 8 }} />
                    <CustomText style={[globalStyles.f12Bold, { color: '#000' }]}>Use Current Location</CustomText>
                </TouchableOpacity>
                ...
                {address ? (
                    <View>
                        ...
                        >
                            ...
                            <View style={{ flex: 1 }}>
                                <CustomText style={{ ...globalStyles.f10Bold, color: '#000' }}>{address}</CustomText>
                            </View>
                ...
                <View style={styles.mapContainer}>
                    {region ? (
                        ...
                        <View style={styles.mapLoading}>
                            <CustomText style={[globalStyles.f12Bold, { color: '#000' }]}>Loading Map...</CustomText>
                        </View>
                    )}
                </View>
                        ...
                        >
                            <View style={styles.modalContent}>
                                ...
                                >
                                    ...
                                    />
                                    {addressLine1Error && (
                                        <CustomText style={[{ color: 'red', marginTop: -12, marginBottom: 10 }, globalStyles.f10Regular]}>
